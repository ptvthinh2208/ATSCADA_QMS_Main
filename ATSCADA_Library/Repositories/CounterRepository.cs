    ...
    public class CounterRepository : ICounterRepository
    {
        ...
        public async Task<CallNextResult> CallNextTicketAtomicAsync(int counterId, int serviceId)
        {
            ...
            try
            {
                ...
                // Lấy thông tin Counter để biết mốc số hiện tại
                var counter = await _context.Counters.FindAsync(counterId);
                if (counter == null) throw new Exception($"Không tìm thấy Quầy có ID {counterId}");
                ...
                LIMIT 1
                FOR UPDATE
            ", today, serviceId, counter.CurrentNumber)
                    .FirstOrDefaultAsync();
                ...
                // Cập nhật số hiện tại của Counter để đồng bộ LED và làm mốc cho lần gọi tiếp theo
                counter.CurrentNumber = nextClient.OrderNumber;
        ...
        public async Task<CallNextResult> CallPreviousTicketAtomicAsync(int counterId, int serviceId)
        {
            await using var transaction = await _context.Database.BeginTransactionAsync();
            try
            {
                ...
                // Tìm số nhỏ hơn số hiện tại (Gần nhất)
                var prevClient = await _context.Queues
                    .FromSqlRaw(@"
                SELECT * FROM Queues 
                WHERE ServiceId = {1}
                  AND OrderNumber < {2}
            ...
            ", today, serviceId, counter.CurrentNumber)
                ...
                prevClient.CounterId = counterId;
                prevClient.Status = "Processing";
                prevClient.LastTimeUpdated = DateTime.Now;

                // Cập nhật số hiện tại của Counter làm mốc lùi
                counter.CurrentNumber = prevClient.OrderNumber;
            ...
            {
                await transaction.RollbackAsync();
                throw new Exception("Lỗi khi gọi khách phía trước: " + ex.Message, ex);
            }
        }
