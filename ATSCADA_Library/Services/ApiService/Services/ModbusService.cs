...
namespace ATSCADA_Library.Services.ApiService.Services
{
    public class ModbusService : IModbusService
    {
        ...
        {
            // Tăng timeout tổng lên 5s để đảm bảo đủ thời gian Connect và Write
        ...
        private async Task InternalTransmit(int modbusId, ushort newValue, CancellationToken ct)
        {
            ...
            // Chờ Lock để đảm bảo không có 2 request cùng ghi vào 1 bảng LED cùng lúc
            if (!await session.Lock.WaitAsync(TimeSpan.FromSeconds(4), ct))
                throw new TimeoutException("Hệ thống Modbus đang bận, vui lòng thử lại sau.");

            try
            {
                // Kiểm tra kết nối, nếu mất thì thử kết nối lại
                if (!session.Client.Connected)
                {
                    await Task.Run(() => session.Client.Connect());
                }

                // ===== CACHE CHECK =====
                // Nếu giá trị mới giống giá trị cũ đã hiện trên LED thì không cần gửi lại để tiết kiệm băng thông
                if (_lastSentValue.TryGetValue(modbusId, out ushort oldValue) && oldValue == newValue)
                    return;

                // ===== WRITE MODBUS =====
                session.Client.WriteSingleRegister(0, newValue);

                // Lưu lại giá trị cuối cùng đã gửi thành công
                _lastSentValue[modbusId] = newValue;
            }
            catch (Exception ex)
            {
                // Nếu lỗi, xóa cache và xóa session để lần sau khởi tạo lại sạch sẽ
                ...
                throw new Exception($"Lỗi truyền tin tới Modbus ID {modbusId}: {ex.Message}");
            }
            finally
            {
        ...
        private async Task<ModbusSession> GetOrCreateSession(int modbusId)
        {
            ...
            if (cfg == null)
                throw new KeyNotFoundException($"Không tìm thấy cấu hình Modbus ID {modbusId}");

            var client = new ModbusClient(cfg.IpAddress, 502)
            {
                UnitIdentifier = (byte)cfg.SlaveId,
                ConnectionTimeout = 2000 // Timeout kết nối 2s
            };

            // Thực hiện kết nối trong Task.Run để không làm nghẽn luồng xử lý chính
            try
            {
            ...
            {
                throw new Exception($"Không thể kết nối tới bảng LED (IP: {cfg.IpAddress}): {ex.Message}");
            }
