...
namespace ATSCADA_Client.Services.ApiClientService
{
    public class QueueApiClient : IQueueApiClient
    {
        ...
        {
            // Định nghĩa URL API
            string url = $"api/Queue/update-transfer-queue/single/{newServiceID}?counterId={counterId}";
        ...
        public async Task<List<Queue?>> UpdateTransferQueueAsync(long newServiceID, int counterId, List<Queue> request)
        {
            // Định nghĩa URL API
            string url = $"api/Queue/update-transfer-queue/list/{newServiceID}?counterId={counterId}";
            ...
            {
                // Đọc kết quả trả về từ API
                return await response.Content.ReadFromJsonAsync<List<Queue>>();
            }
        ...
        public async Task<Queue?> CallNextNumberAsync(int serviceId, int counterId)
        {
            try
            {
                var request = new { ServiceId = serviceId }; // JSON: { "ServiceId": 1 }

                var response = await _httpClient.PostAsJsonAsync(
                    $"api/queue/counter/{counterId}/call-next",
                    request
                );
                ...
                // Đọc JSON → deserialize thẳng vào class chứa Queue
                var result = await response.Content.ReadFromJsonAsync<CallNextResponse>();
                return result?.Success == true ? result.Client : null;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"CallNextNumber Error: {ex.Message}");
                return null;
            }
        }
        #endregion
