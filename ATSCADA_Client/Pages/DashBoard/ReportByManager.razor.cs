...
{
    public partial class ReportByManager
    {
        ...
        private async Task GetAllQueueHistories()
        {
            if (!string.IsNullOrEmpty(ServiceId))
            {
                ...
                listQueueHistories = listPagingResponse.Items.Where(x=>x.ServiceId == converToLong)
                    .GroupBy(x => new { x.FullName, x.PhoneNumber }).Select(group => group.First()).ToList();
